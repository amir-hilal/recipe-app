        ...
        <nav className="bg-white shadow-lg w-full">
            <div className="max-w-6xl mx-auto px-4">
                <div className="flex justify-between">
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        {!isLoggedIn && (
                            <>
                                <Link to={routes.login} className="py-2 px-4 text-xl bg-green-500 text-white rounded hover:bg-green-600 transition duration-300">Login</Link>
                                <Link to={routes.register} className="py-2 px-4 text-xl bg-gray-500 text-white rounded hover:bg-gray-600 transition duration-300">Register</Link>
                            </>
                        )}
                        {isLoggedIn && (
                            <>
                                <button onClick={handleLogout} className="py-2 px-4 text-xl bg-red-500 text-white rounded hover:bg-red-600 transition duration-300">Logout</button>
                            </>
                        )}
                    </div>
                </div>
            ...
            {isOpen && (
                <div className="md:hidden absolute bg-white shadow-md w-full z-50">
                    ...
                    {!isLoggedIn && (
                        <>
                            <Link to={routes.login} className="block py-4 px-6 text-lg text-gray-700 hover:bg-gray-200">Login</Link>
                            <Link to={routes.register} className="block py-4 px-6 text-lg text-gray-700 hover:bg-gray-200">Register</Link>
                        </>
                    )}
                    {isLoggedIn && (
                        <button onClick={handleLogout} className="block py-4 px-6 text-lg text-red-500 hover:bg-gray-200">Logout</button>
                    )}
                </div>
            )}
        </nav>
